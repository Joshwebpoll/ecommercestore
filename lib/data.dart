final List<Map<String, dynamic>> categoryProduct = [
  {
    "name": 'Phone',
    "image":
        "https://images.pexels.com/photos/248528/pexels-photo-248528.jpeg?auto=compress&cs=tinysrgb&w=600"
  },
  {
    "name": 'Console',
    "image":
        "https://images.pexels.com/photos/1365795/pexels-photo-1365795.jpeg?auto=compress&cs=tinysrgb&w=600"
  },
  {
    "name": 'Laptop',
    "image":
        "https://images.pexels.com/photos/205421/pexels-photo-205421.jpeg?auto=compress&cs=tinysrgb&w=600"
  },
  {
    "name": 'Cameras',
    "image":
        "https://images.pexels.com/photos/3497060/pexels-photo-3497060.jpeg?auto=compress&cs=tinysrgb&w=600"
  },
  {
    "name": 'Desktop',
    "image":
        "https://images.pexels.com/photos/572056/pexels-photo-572056.jpeg?auto=compress&cs=tinysrgb&w=600"
  },
];

final List<Map<String, dynamic>> electronicProducts = [
  {
    "id": 1,
    "name": "Smartphone",
    "brand": "TechBrand",
    "price": 699.99,
    "quantity": 1,
    "category": "Mobile",
    "stock": 120,
    "features": ["5G", "128GB Storage", "AMOLED Display"],
    "rating": 4.5,
    "description":
        'It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout.',
    "image":
        "https://cdn.pixabay.com/photo/2016/03/27/19/43/samsung-1283938_1280.jpg"
  },
  {
    "id": 2,
    "name": "Laptop",
    "brand": "ComputeX",
    "price": 1299.99,
    "quantity": 1,
    "category": "Computers",
    "stock": 60,
    "features": ["16GB RAM", "512GB SSD", "Intel i7"],
    "rating": 4.8,
    "description":
        'It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout.',
    "image":
        "https://cdn.pixabay.com/photo/2016/03/27/07/12/apple-1282241_1280.jpg"
  },
  {
    "id": 3,
    "name": "Bluetooth Speaker",
    "brand": "SoundTech",
    "price": 49.99,
    "quantity": 1,
    "category": "Audio",
    "stock": 200,
    "features": ["Waterproof", "10-hour Battery Life", "Compact Design"],
    "rating": 4.2,
    "description":
        'lorem20  It is a long established  It is a long established   It is a long established It is a long established   It is a long established It is a long established   It is a long established It is a long established   It is a long established It is a long established   It is a long established It is a long established   It is a long established It is a long established   It is a long established It is a long established   It is a long established It is a long established   It is a long established It is a long establishedIt is a long established   It is a long established It is a long established   It is a long established It is a long established   It is a long established It is a long established   It is a long established It is a long established   It is a long established It is a long established   It is a long established It is a long established   It is a long established It is a long established   It is a long established    It is a long established It is a long established   It is a long established  It is a long established It is a long established It is a long established It is a long established It is a long established It is a long establishedIt is a long established It is a long established It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout.',
    "image":
        "https://cdn.pixabay.com/photo/2018/09/17/14/27/headphones-3683983_1280.jpg"
  },
  {
    "id": 4,
    "name": "Smart TV",
    "brand": "ViewPoint",
    "price": 999.99,
    "quantity": 1,
    "category": "Home Entertainment",
    "stock": 30,
    "features": ["4K UHD", "HDR10+", "55-inch Display"],
    "rating": 4.7,
    "description":
        'It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout.',
    "image":
        "https://cdn.pixabay.com/photo/2018/12/22/03/27/smart-tv-3889141_1280.png"
  },
  {
    "id": 5,
    "name": "Wireless Headphones",
    "brand": "AudioPlus",
    "price": 199.99,
    "quantity": 1,
    "category": "Audio",
    "stock": 80,
    "features": ["Noise Cancelling", "30-hour Battery", "Bluetooth 5.0"],
    "rating": 4.6,
    "description":
        'It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout.',
    "image":
        "https://cdn.pixabay.com/photo/2018/09/17/14/27/headphones-3683983_1280.jpg"
  },
  {
    "id": 6,
    "name": "Gaming Console",
    "brand": "PlayWare",
    "price": 499.99,
    "quantity": 1,
    "category": "Gaming",
    "stock": 50,
    "features": ["4K Gaming", "1TB Storage", "VR Compatible"],
    "rating": 4.9,
    "description":
        'It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout.',
    "image":
        "https://cdn.pixabay.com/photo/2024/05/24/16/40/ai-generated-8785422_1280.jpg"
  },
  {
    "id": 7,
    "name": "Smartwatch",
    "brand": "Wristify",
    "price": 249.99,
    "quantity": 1,
    "category": "Wearable",
    "stock": 150,
    "features": ["Heart Rate Monitor", "GPS", "7-day Battery"],
    "rating": 4.4,
    "description":
        'It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout.',
    "image":
        "https://cdn.pixabay.com/photo/2023/10/07/14/24/smartwatch-8300238_1280.jpg"
  },
  {
    "id": 8,
    "name": "Tablet",
    "brand": "TabMaster",
    "price": 399.99,
    "quantity": 1,
    "category": "Mobile",
    "stock": 90,
    "features": ["10.5-inch Display", "256GB Storage", "Stylus Support"],
    "rating": 4.3,
    "description":
        'It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout.',
    "image":
        "https://cdn.pixabay.com/photo/2020/08/26/14/29/smartphone-5519653_1280.jpg"
  },
  {
    "id": 9,
    "name": "Drone",
    "brand": "SkyHigh",
    "price": 799.99,
    "quantity": 1,
    "category": "Camera",
    "stock": 20,
    "features": ["4K Video", "GPS Tracking", "30-minute Flight Time"],
    "rating": 4.6,
    "description":
        'It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout.',
    "image":
        "https://cdn.pixabay.com/photo/2015/12/07/14/18/drone-1080844_1280.jpg"
  },
  {
    "id": 10,
    "name": "Portable Charger",
    "brand": "PowerMax",
    "price": 29.99,
    "quantity": 1,
    "category": "Accessories",
    "stock": 300,
    "features": ["20,000mAh", "Fast Charging", "Dual USB Ports"],
    "rating": 4.1,
    "description":
        'It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout.',
    "image":
        "https://cdn.pixabay.com/photo/2017/08/10/04/14/iphone-2618080_1280.jpg"
  },
];
